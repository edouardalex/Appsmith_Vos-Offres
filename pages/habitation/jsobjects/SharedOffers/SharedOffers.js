...
async initPage() {
    ...
    // DEBUG : On va logger tout l'objet pour voir la vraie structure
    console.log("Structure complète du Query :", getSharedPotentialOffers.data);

    this.offersData = this.prepareOffersData(
        // Teste avec un seul .data si le double .data échoue
        getSharedPotentialOffers.data?.getSharedPotentialOffers || getSharedPotentialOffers.data.data?.getSharedPotentialOffers
    );
	...
	async confirmOfferChoice() {
		...
		try {
			...
			navigateTo("confirmation habitation", {});
		} catch (error) {
			showAlert(
